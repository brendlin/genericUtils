...
This macro offers a way to compare different cut selection. Right now it only works for --signal and --data.
To use it, make a config file and define inside a dictionary called "cutcomparisons", e.g. to compare
different channels:
...
Note that the key of the dictionary corresponds to the name of the selection, and the subsequent
cuts are applied as usual. Then run e.g. the following:
